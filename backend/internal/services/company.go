...
func (s *CompanyService) UpdateCompany(
	...
	// 引数の取得
	newGrpcCompany := req.Msg.GetCompany()
	if newGrpcCompany == nil {
		return nil, connect.NewError(connect.CodeInvalidArgument, errors.New("company is required"))
	}
	newCompany := &models.Company{
		Company: newGrpcCompany,
	}

	// 既存の会社情報を取得
	oldId := req.Msg.GetCompany().GetId()
	oldCompany, exist := s.companiesById[oldId]
	if !exist {
		return nil, connect.NewError(connect.CodeNotFound, errors.New("company not found"))
	}

	// 会社情報を更新
	newCompany, err = oldCompany.Update(newCompany)
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}

	// 会社情報のインデックスを更新
	if _, exist := s.companiesById[oldId]; exist {
		delete(s.companiesById, oldId)
		// 新しいIDで再登録
		s.companiesById[newCompany.GetId()] = newCompany
	}

	response.Msg.SetCompanyMapById(s.companiesById)
